...
local library = {design = getgenv().design == "kali" and "kali" or "uwuware", tabs = {}, draggable = true, flags = {}, title = "orbware", open = false, mousestate = inputService.MouseIconEnabled, popup = nil, instances = {}, connections = {}, options = {}, notifications = {}, tabSize = 0, theme = {}, foldername = "uw_configs", fileext = ".uw"}
...
library.createColorPickerWindow = function(option)
	...
	local Gradient = library:Create("UIGradient", {
		Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0, Color3.fromRGB(131, 111, 182)), -- MENU COLOR?
...
library.Aimbot.Run = function(Step)
	...
	for Player, Data in next, Players do
		...
		if (library.flags["Aimbot TeamCheck"] or Data.Enemy) then
...
AimbotTargeting:AddToggle({text = "Wallcheck", flag = "Aimbot Vis Check", callback = function(State)
	if State then
...
--	end
--end})
AimbotTargeting:AddToggle({text = "TeamCheck", flag = "Aimbot TeamCheck"})
...
TriggerbotMain:AddToggle({text = "TeamCheck", flag = "Triggerbot Teammates"})
			...
			if Show and Character then
				...
				if Health > 0.1 then
					...
					if Distance < library.flags["Esp Max Distance"] then
						...
						local Visible = true
						if library.flags[Team .. " Wallcheck"] then
							Visible = Hit ~= nil
						end
...
PlayerEspEnemySection:AddToggle({text = "Wallcheck", flag = "Enemy Wallcheck"})
...
PlayerEspTeamSection:AddToggle({text = "Wallcheck", flag = "Team Wallcheck"})
